...
const Profile = () => {
  ...
  // useEffect(() => {
  //   const convertBufferToBase64 = async () => {
  //     if (user?.image?.data) {
  //       const byteArray = new Uint8Array(user.image.data);
  //       // SAFELY convert using Blob and FileReader
  //       const blob = new Blob([byteArray], { type: "image/jpeg" }); // change type if needed

  //       const reader = new FileReader();
  //       reader.onloadend = () => {
  //         setImageSrc(reader.result); // already data:image/... format
  //       };
    ...
    <div className=" w-full h-full flex flex-col items-center py-16 ">
      <div className="w-full bg-white dark:bg-[#27282F] relative rounded-3xl shadow-xl p-10">
        <div className="mb-5">
          ...
          <h2 className="text-sm text-slate-400">{new Date(user?.createdAt).toLocaleDateString()}</h2>
        </div>
