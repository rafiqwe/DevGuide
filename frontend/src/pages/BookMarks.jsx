...
const BookMarks = () => {
  ...
  const handleRemove = async (videoId) => {
    try {
      await API.delete(`/api/bookmark/${videoId}`);
      // Update query cache
      ...
      <>
        <div className="flex flex-col items-center  justify-center h-full p-6 text-gray-500 dark:text-gray-400">
          ...
          <p className="max-w-xs text-center">
            Save videos to Book mark and they'll appear here.
          </p>
        </div>
        <ProtectedPage />
      </>
          ...
          {data.map(({ _id, videoId, title, thumbnail, duration }) => (
            ...
            >
              ...
              <button
                ...
                className="absolute cursor-pointer top-3 right-3 bg-red-600 hover:bg-red-700 text-white rounded-full p-2 shadow-lg transition"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-5 w-5"
